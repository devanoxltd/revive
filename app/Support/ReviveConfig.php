...
class ReviveConfig
{
    ...
    {
        $includePaths = static::expandWildcards($config['include'] ?? []);
        ...
        $config['include'] = collect($includePaths)->reject(function ($includePath) use ($excludePaths) {
            return $excludePaths->contains(function ($excludePath) use ($includePath) {
                return Str::of($includePath)->startsWith($excludePath);
            });
        })->values()->toArray();
    ...
    public static function loadLocal(): array
    {
        if (file_exists(Project::path() . '/revive.json')) {
            return tap(json_decode(file_get_contents(Project::path() . '/revive.json'), true, 512, JSON_THROW_ON_ERROR), function ($configuration) {
