...
class PhpCodeSniffer extends Tool
{
    ...
    public function fix(): int
    {
        ...
        if ($lint !== 0) {
            $this->failure('PHP Code_Sniffer found errors that cannot be fixed automatically. Run `revive lint` for details.');
        }
    ...
    private function resolvePaths(): ?array
    {
        if ($this->hasExplicitPaths()) {
            $paths = $this->filterBladeFiles($this->reviveConfig->get('paths'));
